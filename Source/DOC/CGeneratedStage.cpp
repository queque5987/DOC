	...
	for (int32 i = 0; i < 4; i++)
	{
		for (int32 j = 0; j < 4; j++)
		{
				...
				)
			{
				...
				Stage_Room_Coord[x][y].Generated_Room = Cast<IIStageGrid_Meshes>(Stage_Grid_Meshes[STAGE_GRID_MESH_STAIR]);
			...
			if (bQualify)
			{
				for (int32 ydepth = -2; ydepth < 3; ydepth++)
				{
					...
					for (int32 xdepth = 0; xdepth < 5; xdepth++)
					{
						...
						if (ydepth == 0 && xdepth == 2) BossRoomCenterCoordinate = FCoordinate{ depth_x , depth_y, 0 };
			...
			else if (Stage_Room_Coord[i][j].State == ROOM_CORRIDOR || Stage_Room_Coord[i][j].State == ROOM_OCCUPIED)
			{
				...
				for (const FCoordinate& dir : Directions)
				{
						...
						)
					{
						...
						// Actor Door
						Stage_Room_Coord[i][j].DoorTransformArr.Add(
							FTransform(
								FRotator(0.f, 90.f * d, 0.f),
								FVector(
									i * 100.f * Stage_Scale + dir.Get<0>() * 100.f * Stage_Scale / 2,
						...
						Stage_Room_Coord[i][j].DoorArr.Add(nullptr);
						Stage_Room_Coord[i][j].DoorOpenStateArr.Add(INTERACTABLE_ITEM_STATE_CLOSED);
						// Wall Door
						Stage_Room_Coord[i][j].SpawnedComponents.Add(FSpawnedStaticMeshComponent(
							ObjectPoolManager->GetStaticMeshComponent(
